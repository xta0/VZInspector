...
#import "TBCitySandBoxRootView.h"
#import "TBCitySandBoxSubView.h"


@interface TBCitySandBoxHeaderView : UIView<TBCitySandBoxSubViewCallBackProtocol>
...
@implementation TBCitySandBoxHeaderView
...
@interface TBCitySandBoxRootView()<TBCitySandBoxSubViewCallBackProtocol>

@property(nonatomic,strong)NSMutableArray* stack;
@property(nonatomic,strong)TBCitySandBoxHeaderView* headerView;
@property(nonatomic,strong)NSString* homepath;
@property(nonatomic,strong)NSString* filepath;
@property(nonatomic,strong)NSString* filename;
@property(nonatomic,strong)TBCitySandBoxSubView* currentView;

@end

@implementation TBCitySandBoxRootView
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.headerView = [[TBCitySandBoxHeaderView alloc]initWithFrame:CGRectMake(0, 0, frame.size.width, 44)];
...
- (void)loadRootView
{
    TBCitySandBoxSubView* rootView = [[TBCitySandBoxSubView alloc]initWithFrame:CGRectMake(0, 44, self.frame.size.width, self.frame.size.height-44) Dir:self.homepath];
...
- (void)push
{
    TBCitySandBoxSubView* targetView = [[TBCitySandBoxSubView alloc]initWithFrame:CGRectMake(self.frame.size.width, 44, self.frame.size.width, self.frame.size.height-44) Dir:self.filepath];
    targetView.delegate = self;
    targetView.alpha = 0.0f;
    [self addSubview:targetView];
    
    
    TBCitySandBoxSubView* currentView = self.currentView;
...
- (void)pop
{
    if (self.stack.count <2) {
        ...
        [self.superview.viewController performSelector:@selector(onClose)];
        return;
    }
    ...
    TBCitySandBoxSubView* belowView = self.stack[self.stack.count - 2];
