...
- (void)setupScrollView {
    ...
    UILabel *enableLabel = [self labelWithText:@"Start" frame:CGRectMake(8, 0, 36, lineHeight)];
    ...
    _delayLabel = [self labelWithText:[NSString stringWithFormat:@"Delay:%@s", @(fakeConfig.delay)] frame:CGRectMake(105, enableLabel.frame.origin.y, 90, lineHeight)];
    ...
    UILabel *latitudeLabel = [self labelWithText:@"Lat" frame:CGRectMake(enableLabel.frame.origin.x, enableLabel.frame.origin.y + enableLabel.frame.size.height, 30, lineHeight)];
    ...
    UILabel *longitudeLabel = [self labelWithText:@"Lng" frame:CGRectMake(_scrollView.frame.size.width / 2.0 + latitudeLabel.frame.origin.x, latitudeLabel.frame.origin.y, latitudeLabel.frame.size.width, lineHeight)];
...
- (NSArray *)locationList {
    ...
    locations = [locations arrayByAddingObjectsFromArray:
                 @[
                   [[VZLocationItem alloc] initWithCity:@"Hangzhou" latitude:30.26667 longitude:120.2],
                   [[VZLocationItem alloc] initWithCity:@"Beijing" latitude:39.9 longitude:116.3833],
                   [[VZLocationItem alloc] initWithCity:@"Shanghai" latitude:31.24 longitude:121.47],
                   [[VZLocationItem alloc] initWithCity:@"Guangzhou" latitude:23.18 longitude:113.28],
                   [[VZLocationItem alloc] initWithCity:@"NYC" latitude:40.67 longitude:-73.94],
                   [[VZLocationItem alloc] initWithCity:@"Tokyo" latitude:35.7 longitude:139.7],
                   [[VZLocationItem alloc] initWithCity:@"DC" latitude:38.889931 longitude:-77.009003],
                   ]];
    return locations;
}
...
- (void)sliderValueDidChange:(UISlider *)sender {
    ...
    _delayLabel.text = [NSString stringWithFormat:@"Delay:%@s", @(sender.value)];
}
