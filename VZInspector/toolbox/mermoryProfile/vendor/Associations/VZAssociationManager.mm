...
#import "VZAssociationManager+Internal.h"
#import "VZRetainCycleDetector.h"

#import "vzfishhook.h"
#import "VZInspectorThread.h"
#import "VZAssociationManager.h"
#import "VZAllocationTrackerManager.h"

#if _INTERNAL_RCD_ENABLED

namespace VZ { namespace AssociationManager {
  using ObjectAssociationSet = std::unordered_set<void *>;
  using AssociationMap = std::unordered_map<id, ObjectAssociationSet *>;

    static std::unordered_map<id, std::unordered_set<void *> *> *_associationMap;

  static bool hookTaken ;
   
  void init(){
      _associationMap = new std::unordered_map<id, std::unordered_set<void *> *>;
      hookTaken = false;
  ...
  void _threadUnsafeResetAssociationAtKey(id object, void *key) {
   ...
   static  std::mutex *hookMutex(){
      static std::mutex *_hookMutex(new std::mutex);
      return _hookMutex;
  }
    
    static  std::mutex *associationMutex(){
        static std::mutex *_associationMutex(new std::mutex);
        return _associationMutex;
    }
  ...
  NSArray *associations(id object) {
    std::lock_guard<std::mutex> l(*associationMutex());
    if (_associationMap->size() == 0 ){
      return nil;
    }
  ...
  static void (*vz_orig_objc_setAssociatedObject)(id object, void *key, id value, objc_AssociationPolicy policy);
  static void (*vz_orig_objc_removeAssociatedObjects)(id object);

  static void vz_objc_setAssociatedObject(id object, void *key, id value, objc_AssociationPolicy policy) {
    {
      std::lock_guard<std::mutex> l(*associationMutex());
    ...
    vz_orig_objc_setAssociatedObject(object, key, value, policy);
  }

  static void vz_objc_removeAssociatedObjects(id object) {
    {
      std::lock_guard<std::mutex> l(*associationMutex());
      _threadUnsafeRemoveAssociations(object);
    }

    vz_orig_objc_removeAssociatedObjects(object);
  }
...
@implementation VZAssociationManager

+ (void)hook
{
#if _INTERNAL_RCD_ENABLED
    VZ::AssociationManager::init();
  std::lock_guard<std::mutex> l(*VZ::AssociationManager::hookMutex());
  vz_rcd_rebind_symbols((struct rcd_rebinding[2]){
    {
      "objc_setAssociatedObject",
      (void *)VZ::AssociationManager::vz_objc_setAssociatedObject,
      (void **)&VZ::AssociationManager::vz_orig_objc_setAssociatedObject
    },
    {
      "objc_removeAssociatedObjects",
      (void *)VZ::AssociationManager::vz_objc_removeAssociatedObjects,
      (void **)&VZ::AssociationManager::vz_orig_objc_removeAssociatedObjects
    }}, 2);
  VZ::AssociationManager::hookTaken = true;
...
#if _INTERNAL_RCD_ENABLED
  std::lock_guard<std::mutex> l(*VZ::AssociationManager::hookMutex());
  if (VZ::AssociationManager::hookTaken) {
    vz_rcd_rebind_symbols((struct rcd_rebinding[2]){
      {
        "objc_setAssociatedObject",
        (void *)VZ::AssociationManager::vz_orig_objc_setAssociatedObject,
...
{
#if _INTERNAL_RCD_ENABLED
  return VZ::AssociationManager::associations(object);
#else
  return nil;
#endif //_INTERNAL_RCD_ENABLED
}
