...
@implementation VZObjectiveCObject
...
- (NSArray *)_unfilteredRetainedObjects
{
  ...
  NSArray *strongIvars = VZGetObjectStrongReferences(self.object, self.configuration.layoutCache);

  NSMutableArray *retainedObjects = [[[super allRetainedObjects] allObjects] mutableCopy];

   NSString *namePathClassName = NSStringFromClass([self.object class]);
  for (id<VZObjectReference> ref in strongIvars) {
    id referencedObject = [ref objectReferenceFromObject:self.object];

    if (referencedObject) {
      NSArray<NSString *> *namePath = [ref namePath];
      [retainedObjects addObject:VZWrapObjectGraphElementWithContext(referencedObject,
                                                                     self.configuration,
                                                                     namePath,
                                                                     namePathClassName)];
    }
    ...
    for (NSInteger i = 0; i < tries; ++i) {
      ...
      @try {
        for (id subobject in self.object) {
          if (retainsKeys) {
            [temporaryRetainedObjects addObject:VZWrapObjectGraphElementWithContext(subobject, self.configuration,
                                                                                    @[@"key"],
                                                                                    namePathClassName)];
          }
          if (isKeyValued && retainsValues) {
            [temporaryRetainedObjects addObject:VZWrapObjectGraphElementWithContext([self.object objectForKey:subobject],
                                                                         self.configuration,
                                                                                    @[@"value"],
                                                                                    namePathClassName)];
          }
