...
#import "VZBlockInterface.h"
#import "VZBlockStrongRelationDetector.h"
...
static NSIndexSet *_GetBlockStrongLayout(void *block) {
  ...
  for (size_t i = 0; i < elements; ++i) {
    VZBlockStrongRelationDetector *detector = [VZBlockStrongRelationDetector new];
  ...
  for (size_t i = 0; i < elements; ++i) {
    VZBlockStrongRelationDetector *detector = (VZBlockStrongRelationDetector *)(detectors[i]);
...
NSArray *VZGetBlockStrongReferences(void *block) {
  if (!VZObjectIsBlock(block)) {
    return nil;
  }
...
BOOL VZObjectIsBlock(void *object) {
